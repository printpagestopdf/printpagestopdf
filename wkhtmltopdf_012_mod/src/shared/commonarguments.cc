...
//RR
	...
	addarg("replace-body", 0, "Replace body by content of file (local Path)", new QStrSetter(s.replaceBody, "rs"));
	addarg("local-iframe", 0, "Search for iframes, loaded from file", new ConstSetter<bool>(s.localIframe, true));
	addarg("no-local-iframe", 0, "Search for iframes, loaded from file", new ConstSetter<bool>(s.localIframe, false));
	addarg("load-timeout",0,"Cancel page load after n milliseconds", new IntSetter(s.loadTimeout,"msec"));
