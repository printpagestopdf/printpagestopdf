...
PdfCommandLineParser::PdfCommandLineParser(PdfGlobal & s, QList<PdfObject> & ps):
	...
	//RR
	addarg("no-guesspagelength", 0, "Dont try to set pagelength to webpagelength", new ConstSetter<bool>(s.guessPageLength, false));
	addarg("guesspagelength", 0, "Try to set pagelength to webpagelength", new ConstSetter<bool>(s.guessPageLength, true));
