...
struct DLL_LOCAL ReflectImpl<PdfGlobal>: public ReflectClass {
	ReflectImpl(PdfGlobal & c) {
		...
		//RR
		WKHTMLTOPDF_REFLECT(guessPageLength);
...
QPrinter::PrinterMode strToPrinterMode(const char * s, bool * ok) {
	...
	if (ok) *ok=true;
	//RR no strcmp QString == instead
	if (QString(s) == "screen")		return QPrinter::ScreenResolution;
	if (QString(s) == "printer")	return QPrinter::PrinterResolution;
	if (QString(s) == "high")		return QPrinter::HighResolution;
	*ok=false;
...
QPrinter::ColorMode strToColorMode(const char * s, bool * ok) {
	if (ok) *ok=true;
	//RR no strcmp QString == instead
	if (QString(s).toLower() == "color")		return QPrinter::Color;
	if (QString(s).toLower() == "grayscale")	return QPrinter::GrayScale;
	*ok=false;
